...
def main():
    ...
    for file in search_results:
        ...
        for line in f:
            if 'matched' in line:
                ...
                answer_FEI = get_FEI_from_line(line)
                log('\t' + answer_filename)
                if answer_filename == query_filename:
                    if parse_mode == Mode.Neighborhoods:
                        results['neighborhoods-correct-results'] += answer_FEI
                    else:
                        results['naive-correct-results'] += answer_FEI
        ...
        if VERBOSE:
            ...
            print('\tFEI (naive)         :', round(results['naive-correct-results']/results['naive-queries-count'], 2) )
            print('\tFEI (neighborhoods) :', round(results['neighborhoods-correct-results']/results['neighborhoods-queries-count'], 2) )
            print()
    ...
    print('Total FEI (naive)         :', round(all_results['total_naive_correct']/all_results['total_queries'], 3))
    print('Total FEI (neighborhoods) :', round(all_results['total_neighborhoods_correct']/all_results['total_queries'], 3))
    print('=' * PRINT_LEN)
