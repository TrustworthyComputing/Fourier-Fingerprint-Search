...
def fingerprint(stl_file,
                db,
                num_of_slices,
                num_of_peaks_to_keep=DEFAULT_NUM_OF_PEAKS,
    ...
    for i in range(num_of_slices):
        ...
        peaks_filtered = filter(lambda x: x[2] > min_magnitude,
                                peaks)  # freq, time, mag
        ...
        local_maxima = zip(frequency_x_idx, frequency_y_idx,
                           [i for k in range(len(frequency_y_idx))])
